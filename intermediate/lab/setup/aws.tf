...
resource "aws_iam_role" "tfc_role" {
  name = "${random_pet.this.id}-tfc-role"
...
resource "aws_iam_policy" "tfc_policy" {
  name        = "${random_pet.this.id}-tfc-policy"
 ...
 "Statement": [
   {
     ...
     "Resource": [
       ...
       "arn:aws:secretsmanager:*:*:tfe-training*"
     ],
     "Condition": {
     "StringEquals": {
...
resource "aws_iam_user" "secrets_engine" {
  name = "${random_pet.this.id}-hcp-vault-secrets-engine"
}
...
resource "aws_iam_user_policy" "vault_secrets_engine_generate_credentials" {
  name = "${random_pet.this.id}-hcp-vault-secrets-engine-policy"
  user = aws_iam_user.secrets_engine.name
